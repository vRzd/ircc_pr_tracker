...
from src.pages.applications_details_page import ApplicationDetailPage
...
class IRCCStatusChecker:
    ...
    def initialize_pages(self):
        ...
        if self.config["features"].get("check_task_count", False):
            ...
            self.pages["application_detail"] = ApplicationDetailPage(self.driver_manager)
        # if self.config["features"].get("check_last_status", False):
        #     self.pages["processing"] = ApplicationDetailPage(self.driver_manager, self.config)
    ...
    def run(self):
        try:
            ...
            if "login" in self.pages and "task" in self.pages:
                ...
                background_verification = self.pages["application_detail"].get_background_verification_status()
            ...
            if self.config["features"].get("report_to_telegram", False):
                self.telegram_notifier.send_message(self.config.get("chat_ids", []), client_message)
